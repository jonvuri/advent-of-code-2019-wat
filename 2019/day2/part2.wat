(module
  ...
  ;; constant string - Out of bounds message
  (data (i32.const 500) "Out of bounds access\n")
  (global $out_of_bounds_msg i32 (i32.const 500))
  (global $out_of_bounds_msg_len i32 (i32.const 21))

  ;; constant string - Debug point 1 message
  (data (i32.const 600) "Debug point 1\n")
  (global $debug_1_msg i32 (i32.const 600))
  (global $debug_1_msg_len i32 (i32.const 14))

  ;; constant string - Debug point 2 message
  (data (i32.const 700) "Debug point 2\n")
  (global $debug_2_msg i32 (i32.const 700))
  (global $debug_2_msg_len i32 (i32.const 14))

  ;; constant string - Failure message
  (data (i32.const 800) "Failed to find good inputs\n")
  (global $failure_msg i32 (i32.const 800))
  (global $failure_msg_len i32 (i32.const 27))
  ...
  ;; working buffer 2
  (global $work_buf2 i32 (i32.const 196608))
  ...
  ;; Prints the contents of a buffer as numbers, separated by newlines.
  (func $print_buffer (param $buf i32) (param $len i32)
    (local $i i32)
    (local $ptr i32)
    ...
    (block $break_outer_loop
      (loop $outer_loop
        (call $println_number (i32.load (local.get $ptr)))

        (local.set $ptr (i32.add (local.get $ptr) (i32.const 4)))

        (i32.eq (local.get $ptr) (i32.add (local.get $buf) (i32.mul (local.get $len) (i32.const 4))))
  ...
  ;; This function parses any file that contains textual integers separated by non-numeric
  ;; characters, storing the integer values in the working buffer.
  ...
  (func $die_if_oob (param $buf i32) (param $len i32) (param $ptr i32)
    (if 
      (i32.or
        ;; Die if ptr is before start of buffer
        (i32.lt_u 
          (local.get $ptr)
          (local.get $buf)
        )
        ;; Die if ptr is after end of buffer
        (i32.gt_u
          (local.get $ptr)
          ;; Assume buffer of i32s (4 bytes)
          (i32.add (local.get $buf) (i32.mul (local.get $len) (i32.const 4)))
        )
      )
      (then
        (call $die
          (global.get $out_of_bounds_msg)
          (global.get $out_of_bounds_msg_len)
        )
      )
    )
  )
  ...
  (func $get_answer (param $input_len i32) (result i32)
    (local $ptr i32)
    (local $opcode i32)
    (local $r1 i32) ;; Register 1 for instructions
    (local $r2 i32) ;; Register 2 for instructions
    (local $ro i32) ;; Output register for instructions

    ;; Noun and verb to test in each iteration
    (local $noun i32) ;; Can be 0-99
    (local $verb i32) ;; Can be 0-99

    (local.set $noun (i32.const 0))
    (local.set $verb (i32.const 0))

    (loop $outer_loop
      ;; At the start of each loop, copy the program memory from work_buf to work_buf2
      (memory.copy (global.get $work_buf2) (global.get $work_buf) (i32.mul (local.get $input_len) (i32.const 4)))

      ;; Start at beginning of work_buf2
      (local.set $ptr (global.get $work_buf2))

      ;; Overwrite positions 1 and 2 with the current noun and verb
      (i32.store (i32.add (local.get $ptr) (i32.const 4)) (local.get $noun))
      (i32.store (i32.add (local.get $ptr) (i32.const 8)) (local.get $verb))

      ;; Now, run the program
      (block $break_loop
        (loop $loop
          (local.set $opcode (i32.load (local.get $ptr)))

          (if ;; if opcode is 99, end the program
            (i32.eq (local.get $opcode) (i32.const 99))
            (then
              (br $break_loop)
            )
          )

          ;; if opcode is 1, add the next two numbers and store in the third
          ;; relative positions:
          ;; 0: opcode
          ;; 1: position of first number
          ;; 2: position of second number
          ;; 3: position to store result
          (if
            (i32.eq (local.get $opcode) (i32.const 1))
            (then
              ;; First, bootstrap r1 with pointer at position 1, then check if pointer is in bounds
              (local.set $r1
                ;; Pointer at position is an offset of i32's from the start of the buffer
                (i32.add
                  (global.get $work_buf2)
                  (i32.mul
              ...
              (call $die_if_oob (global.get $work_buf2) (local.get $input_len) (local.get $r1))
              ;; If not, then set r1 to value at position
              (local.set $r1 (i32.load (local.get $r1)))

              ;; Then do all the same for r2
              (local.set $r2
                (i32.add
                  (global.get $work_buf2)
                  (i32.mul
              ...
              (call $die_if_oob (global.get $work_buf2) (local.get $input_len) (local.get $r2))
              (local.set $r2 (i32.load (local.get $r2)))

              ;; Then do the same for ro, but keep it as a pointer and don't load the value
              (local.set $ro
                (i32.add
                  (global.get $work_buf2)
                  (i32.mul
              ...
              (call $die_if_oob (global.get $work_buf2) (local.get $input_len) (local.get $ro))
              
              ;; Finally, add values at r1 and r2 and store the result at ro
              (i32.store (local.get $ro) (i32.add (local.get $r1) (local.get $r2)))
            )
          )

          ;; if opcode is 2, do all the same as opcode 1, but multiply instead of add
          (if
            ...
            (then
              ...
              (i32.store (local.get $ro) (i32.mul (local.get $r1) (local.get $r2)))
            )
          )

          ;; Nothing left to do - advance to the next opcode 4 i32's forward
          (local.set $ptr (i32.add (local.get $ptr) (i32.const 16)))
          (call $die_if_oob (global.get $work_buf2) (local.get $input_len) (local.get $ptr))

          (br $loop)
        )
      )
    
      ;; Finally, read from memory position 0 - if it's 19690720, return 100 * noun + verb
      (if
        (i32.eq
          (i32.load (global.get $work_buf2))
          (i32.const 19690720)
        )
        (then
          (return
            (i32.add
              (i32.mul (i32.load (i32.add (global.get $work_buf2) (i32.const 4))) (i32.const 100))
              (i32.load (i32.add (global.get $work_buf2) (i32.const 8)))
            )
          )
        )
      )

      ;; Otherwise, set $noun and $verb to the next pair of numbers in (0-99, 0-99), then loop
      (if
        (i32.eq (local.get $verb) (i32.const 99))
        (then
      ...
      ;; Check if we overflowed, and if so, die
      (if
        (i32.eq (local.get $noun) (i32.const 100))
        (then
          (call $die
            (global.get $failure_msg)
            (global.get $failure_msg_len)
          )
        )
      )
    ...
    ;; We should've found the inputs by now, so if we haven't, die
    (call $die
      (global.get $failure_msg)
      (global.get $failure_msg_len)
    )
    (unreachable) ;; Tell the compiler that it's ok we're not returning an i32
  )
