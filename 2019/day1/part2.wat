(module
  ...
  (func $read_file_to_input_buf (param $dirfd i32) (result i32)
  ...
  (func $parse_input_buf_to_working_buf (param $file_size i32) (result i32)
    (local $i i32)
    (local $num i32)
    (local $ptr i32)
    (local $next_num i32)
  ...
  (func $get_answer (param $input_len i32) (result i32)
    ...
    (block $break_loop
      (loop $LOOP
        (local.set $mass (i32.load (local.get $ptr)))
        (local.set $fuel (i32.const 0))

        (block $break_inner_loop
          (loop $inner_loop
            ;; Calculate the fuel needed for the current mass
            (local.set $fuel
              (i32.sub
                ;; TODO: Handle overflow
                (i32.div_s (local.get $mass) (i32.const 3))
                (i32.const 2)
              )
            )
        
            ;; Break if the current fuel is <= 0
            (br_if $break_inner_loop
              (i32.le_s (local.get $fuel) (i32.const 0))
            )

            ;; Otherwise, add fuel to answer and update mass to current fuel
            (local.set $answer
              (i32.add
                (local.get $answer)
                (local.get $fuel)
        ...
        (i32.ge_u (local.get $ptr) (i32.add (global.get $work_buf) (i32.mul (local.get $input_len) (i32.const 4))))
        (br_if $break_loop)
  ...
  (func $main
    ...
    (local.set $file_size (call $read_file_to_input_buf (local.get $dirfd)))

    ;; (call $println_number (local.get $input_len))

    (local.set $input_len
      (call $parse_input_buf_to_working_buf (local.get $file_size))
    )
